...
cd infra
sam build
sam local start-api           # Lambda + API Gateway 로컬 에뮬레이션
```
...
Backend:   API Gateway → Lambda (FastAPI + Mangum) → DynamoDB
...
| 서비스 | 무료 범위 | 비고 |
|--------|----------|------|
| Lambda | 요청 100만/월 + 40만 GB-초 | 무기한 |
| API Gateway | REST 호출 100만/월 | 12개월 |
...
| Backend | Lambda, API Gateway, DynamoDB | API 설계, 비즈니스 로직 |
